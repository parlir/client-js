...
describe("Browser tests", () => {
    ...
    describe ("Complete authorization", () => {
        it ("code flow", async () => {
            ...
            mockServer.mock({ body: tokenResponse });
            ...
            const key = await Storage.get(smart.KEY);
            const savedState = await Storage.get(key);

            expect(key, `must have set a state at ${smart.KEY}`).to.exist();
            expect(sessionStorage.getItem(key), `must have set a state at sessionStorage.${key}`).to.exist();
            expect(savedState.tokenResponse).to.equal(tokenResponse);
            expect(client.getPatientId()).to.equal(tokenResponse.patient);
            expect(client.getEncounterId()).to.equal(tokenResponse.encounter);
        ...
        it ("code flow with fullSessionStorageSupport = false", async () => {
            ...
            expect(JSON.parse(sessionStorage.getItem(key + "") + "").tokenResponse).to.equal(tokenResponse)
            expect(await Storage.get(key), "must have set a state at " + key).to.exist();
    ...
    describe("smart", () => {
        ...
        describe("ready", () => {
            it ("works with onSuccess callback", async () => {
                const env = new BrowserEnv();
                await smart.authorize(env, {
                    fhirServiceUrl: "http://localhost",
                    redirectUri: "https://test.com",
                ...
                let called = false
                await smart.ready(env, () => called = true)
                expect(called).to.be.true()
            });
            it ("works with onError callback", async () => {
                const env = new BrowserEnv();
                await smart.authorize(env, {
                    fhirServiceUrl: "http://localhost",
                    redirectUri: "https://test.com",
        ...
        describe("buildTokenRequest", () => {
            ...
            it ("works with clientSecret", () => {
                // @ts-ignore
                expect(smart.buildTokenRequest("code", {
