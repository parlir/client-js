...
function getSecurityExtensionsFromConformanceStatement(baseUrl = "/", requestOptions) {
    return _1.fetchConformanceStatement(baseUrl, requestOptions).then(meta => {
        ...
        const extensions = (_1.getPath(meta || {}, "rest.0.security.extension") || [])
...
async function authorize(env, params = {}) {
    ...
    if (Array.isArray(params)) {
        ...
        _1.assert(urlISS, 'Passing in an "iss" url parameter is required if authorize uses multiple configurations');
    ...
    // ------------------------------------------------------------------------
    // Obtain input
    const { clientSecret, fakeTokenResponse, patientId, encounterId, noRedirect, target, width, height, multiple } = params;
...
async function getAccessToken(adapter, code, state) {
    ...
    _1.assert(stored.tokenUri, "No tokenUri found for this server");
...
async function completeAuth(env) {
    ...
    _1.assert(key, "No 'state' parameter found. Please launch this as SMART app.");
    ...
    _1.assert(state, "No state found! Please (re)launch the app.");
    ...
    if (!authorized && state.tokenUri) {
        ...
        state.expiresAt = _1.getAccessTokenExpiration(tokenResponse);
